...
и не надо при этом никуда торопиться___
    ...
    fun countSubstrings() {
        assertEquals(
            mapOf("е" to 49, "РАЗНЫЕ" to 2, "ные" to 2, "Неряшливость" to 1, "эволюция" to 0),
            countSubstrings("input/substrings_in1.txt", listOf("е", "РАЗНЫЕ", "ные", "Неряшливость", "эволюция"))
        )
        assertEquals(
 ...
 */"""
    ...
    fun centerFile() {
        ...
        assertFileContent(
            "temp.txt",
            """ БХБАВААГАД / бхбаваагад
                |бХбаваагаД --- БХБАВААГАД,""".trimMargin()
        )
        File("temp.txt").delete()
